...
/**
 * Use the onChange method to describe what happens
 * to the ToolKit when it changes i.e. a new tool is selected from the kit.
 *
 * this.__emitter will pass the currentTool that was selected and
 * the lastTool that was selected as arguments. Which will allow us
 * to bind the new tool to the current layer context and unbind the last tool
 * from the current context.
 *
 * @param  {Function} cb the callback function to use when the change event triggers
 */
ToolKit.prototype.onChange = function (cb) {
  if (cb instanceof Function !== true) throw new Error('callback must be passed as argument for ToolKit.onChange');
