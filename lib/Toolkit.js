...
function ToolKit(width, height) {
  ...
  toolkit.style.position = 'relative';
  toolkit.style.float = 'left';
  toolkit.style.zIndex = '999';
  toolkit.style.boxShadow = '0 0 10px 0 #AAAAAA';
  toolkit.style.border = '1px solid #999999';
  toolkit.style.boxSizing = 'border-box';

  DOMExtendCommon.call(this, toolkit);

  let toolSet = Immutable.Set();
  Object.defineProperty(this, 'toolSet', { get: () => toolSet, set: (newSet) => toolSet = newSet, configurable: false });
};
 ...
 * @return {Tool}
 */
ToolKit.prototype.pickup = function (tool, event) {
  this.toolSet.forEach((_t) => _t.select = false );
  tool.select = true;
};
